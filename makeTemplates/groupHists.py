        ...
        for cat in catList:
                ...
                for dat in samples_data:
                        if samples_data[dat].year not in yearList:
                                continue
                        if isFirstHist:
                                hists = dataHistFile.Get(inPrefix+'_'+samples_data[dat].prefix).Clone(f'{histoPrefix}__data_obs')
                                isFirstHist = False
                        else:
                                ...
                                if isFirstHistDir[year]:
                                        ...
                                        if doAllSys:
                                                for syst in systematicList:
                                                        if 'pdf' in syst:
                                                                if doMuRF:
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}'] = bkgHistFile.Get(f'{inPrefix}_{syst}_{bkgPrefix}').Clone(f'{histoPrefix}__{proc}__{syst}{year}')
                                                                else: # let's add nominal for WW, etc, rather than have nothing...
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}'] = bkgHistFile.Get(f'{inPrefix}_{bkgPrefix}').Clone(f'{histoPrefix}__{proc}__{syst}{year}')
                                                        else:
                                                                try:
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}Up'] = bkgHistFile.Get(f'{inPrefix}_{syst}Up_{bkgPrefix}').Clone(f'{histoPrefix}__{proc}__{syst}{year}Up')
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}Down'] = bkgHistFile.Get(f'{inPrefix}_{syst}Dn_{bkgPrefix}').Clone(f'{histoPrefix}__{proc}__{syst}{year}Down')
                                                                except:                                                                
                                        ...
                                        nomHists[f'{histoPrefix}__{proc}{year}'].Add(bkgHistFile.Get(f'{inPrefix}_{bkgPrefix}'))
                                        if doAllSys:
                                                for syst in systematicList:
                                                        if 'pdf' in syst:
                                                                if doMuRF:
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}'].Add(bkgHistFile.Get(f'{inPrefix}_{syst}_{bkgPrefix}'))
                                                                else:
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}'].Add(bkgHistFile.Get(f'{inPrefix}_{bkgPrefix}'))
                                                        else:
                                                                try:
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}Up'].Add(bkgHistFile.Get(f'{inPrefix}_{syst}Up_{bkgPrefix}'))
                                                                        systHists[f'{histoPrefix}__{proc}__{syst}{year}Down'].Add(bkgHistFile.Get(f'{inPrefix}_{syst}Dn_{bkgPrefix}'))
                                                                except:
                                                                        if ('pNet' in syst and ('untag' in cat or ('Wtag' in syst and 'Tjet' in cat) or ('Ttag' in syst and 'Wjet' in cat))):
                                                                                pass
                                                                        else:
...
for cat in catList:
        ...
        if doABCDnn and 'ttbar' in bkgProcs.keys():
