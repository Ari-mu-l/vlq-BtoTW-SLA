...
dirPostFix = 'BtargetHoleCorrBTrain_smooth_rebin_2016'
...
def add_systematics(cb):
	...
	#### Use these rateParams to make a comparison to 2016-only
        ...
        if isABCDnn:
                ...
                cb.cp().process([allbkgs[0]]).channel(chns1).AddSyst(cb, 'abcdRateC1', 'lnN', ch.SystMap()(1.14)) #1.20
                cb.cp().process([allbkgs[0]]).channel(chns2).AddSyst(cb, 'abcdRateC2', 'lnN', ch.SystMap()(1.09)) #1.15
                cb.cp().process([allbkgs[0]]).channel(chns3).AddSyst(cb, 'abcdRateC3', 'lnN', ch.SystMap()(1.12)) #1.15
                cb.cp().process([allbkgs[0]]).channel(chns4).AddSyst(cb, 'abcdRateC4', 'lnN', ch.SystMap()(1.20)) #1.20
...
if __name__ == '__main__':
        ...
        rfile = fileDir+template+'/templates_'+discrim+'_138fbfb_2016_'+postfix+'.root'
        if 'D' not in region:
