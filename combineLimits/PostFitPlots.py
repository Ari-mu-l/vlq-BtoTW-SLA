...
sig2 = 'BpM1800'
#sig3 = 'BpM1300'
sig1leg = 'B ('+str(mass1val/1000.0)+' TeV, 6.5 fb) x100'
...
for chn in chns:
    ...
    if isSR:
        hsig1merged.Scale(100*100*xsec[sig1[3:]]*0.5) # 100 is to make it visible on plot. 100 is to revert 0.01pb to 1pb.
    else:
    ...
    if not blind or partialUnblind:
        lPad=TPad("lPad","",0,yDiv/2,1,yDiv) #for sigma runner
        lPad.SetTopMargin(0)
        lPad.SetBottomMargin(0) #0.4
