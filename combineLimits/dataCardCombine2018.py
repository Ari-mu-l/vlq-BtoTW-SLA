...
dirPostFix = 'BtargetHoleCorrBTrain_smooth_rebin_2018'
...
def add_systematics(cb):
	...
	#### Use these rateParams to make a comparison to 2016-only
        ...
        if isABCDnn:
                ...
                cb.cp().process([allbkgs[0]]).channel(chns1).AddSyst(cb, 'abcdRateC1', 'lnN', ch.SystMap()(1.04)) #2018 uncert
                cb.cp().process([allbkgs[0]]).channel(chns2).AddSyst(cb, 'abcdRateC2', 'lnN', ch.SystMap()(1.02))
                cb.cp().process([allbkgs[0]]).channel(chns3).AddSyst(cb, 'abcdRateC3', 'lnN', ch.SystMap()(1.01))
                cb.cp().process([allbkgs[0]]).channel(chns4).AddSyst(cb, 'abcdRateC4', 'lnN', ch.SystMap()(1.03))
...
if __name__ == '__main__':
        ...
        rfile = fileDir+template+'/templates_'+discrim+'_138fbfb_2018_'+postfix+'.root'
        if 'D' not in region:
