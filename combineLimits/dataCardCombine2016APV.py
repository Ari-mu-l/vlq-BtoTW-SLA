...
dirPostFix = 'BtargetHoleCorrBTrain_smooth_rebin_2016APV'
template = 'templates'+region+'_Jan2025_105bins'+dirPostFix
postfix = 'smoothedJJ_rebinned1_stat0p2_smoothedTV' # _UC'
saveKey = 'ABCDnn_'+region 
dateKey = '_Jan2025'
outputdir = 'limits_templates'+saveKey+dateKey+'_RB1_2Dcorr'+dirPostFix  ## Edit last string for unique identifier. IF CHANGING BINNING, GO CHANGE FILE NAME BELOW!
...
def add_systematics(cb):
	...
	#### Use these rateParams to make a comparison to 2016-only
        ...
        if isABCDnn:
                ...
                # cb.cp().process([allbkgs[0]]).channel(chns).AddSyst(cb, 'correctMassRange2', 'shape', ch.SystMap()(1.0))
                ...
                cb.cp().process([allbkgs[0]]).channel(chns1).AddSyst(cb, 'abcdRateC1', 'lnN', ch.SystMap()(1.05)) # 2016APV alpha ratio uncert
                cb.cp().process([allbkgs[0]]).channel(chns2).AddSyst(cb, 'abcdRateC2', 'lnN', ch.SystMap()(1.03))
                cb.cp().process([allbkgs[0]]).channel(chns3).AddSyst(cb, 'abcdRateC3', 'lnN', ch.SystMap()(1.04))
                cb.cp().process([allbkgs[0]]).channel(chns4).AddSyst(cb, 'abcdRateC4', 'lnN', ch.SystMap()(1.17))
                        ...
                        if 'ABCDnn' in saveKey:
                                with open('combined.txt.cmb', 'a') as chnfile:
                                        for reg in regionlist:
                                                ...
                                                # chnfile.write('nuisance edit rename major Case1_'+reg+' trainMassRange3 abcdTrainMassRange3C1\n')
                                                ...
                                                # chnfile.write('nuisance edit rename major Case1_'+reg+' correctMassRange4 abcdCorrMassRange4C1\n')
                                                # chnfile.write('nuisance edit rename major Case2_'+reg+' correctMassRange4 abcdCorrMassRange4C2\n')
                                                if not boosted:
                                        ...
                                        chnfile.write('nuisance edit rename ttx * muRFcorrdNewTTX muRFttx\n')
                                        chnfile.write('nuisance edit rename BpM * muRFcorrdNewSIG muRFsig\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * TrigEffEl TrigEl\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * TrigEffMu TrigMu\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * jec jec\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * jer jer\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * btagHFUC bHFUC\n')
                                        chnfile.write('nuisance edit rename (ttx|ewk|BpM) * btagLFUC bLFUC\n')
...
if __name__ == '__main__':
        ...
        rfile = fileDir+template+'/templates_'+discrim+'_138fbfb_2016APV_'+postfix+'.root'
        if 'D' not in region:
